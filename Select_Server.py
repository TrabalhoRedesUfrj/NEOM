            ...
            if sock == server_socket.soc:
                ...
                try:
                        ...
                        if "authenticate" in commands:
                            ...
                            for ad in usernames.keys():
                                if check['user'] == usernames[ad]:
                                    out.addOther("fail")
                                    out.addMessage("Usuario ja esta conectado, não e possivel reconectar.")
                            ...
                            if "add" in commands:
                                ...
                                if text == "User removed":
                                    ...
                                    if text == "Wrong Password":
                                        ...
                                        out.addMessage("Usuario não encontrado. Tente novamente." % (text))
                                    ssl_sock.send(out.sendMessage())
                                    ssl_sock.close()
                            else:
                    ...
                    if connect:
                        ...
                        out.addName("Server")
                        out.addOther("userIn")
                        out.addMessage("%s %s agora está disponivel\n" % (usernames[addr],str(addr)))
