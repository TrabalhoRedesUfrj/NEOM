...
class ChatServer(threading.Thread):
    ...
    def run(self):
        ...
        while True:
            ...
            criptosocket =ssl.wrap_socket(connectionsocket,server_side=True,certfile="server.crt",keyfile="server.key")
            try:
                threading.Thread(target=self.run_thread, args=(criptosocket, addr)).start()
            finally:
                #criptosocket.shutdown(socket.SHUT_RDWR) #Pode conter erros
                criptosocket.close()
